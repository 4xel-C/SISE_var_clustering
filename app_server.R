...
app_server <- function(input, output, session) {
  ...
  output$inertia_plot <- shiny::renderPlot({
    ...
    tryCatch({
      # Use native plot_elbow method from KmeansVariables
      rv$model$plot_elbow()
    }, error = function(e) {
      plot.new()
      text(0.5, 0.5, paste("Error:", e$message), col = "red")
  ...
  output$kmeans_projection_plot <- shiny::renderPlot({
    ...
    tryCatch({
      # Use native plot_projection method from KmeansVariables
      rv$model$plot_projection()
    }, error = function(e) {
      plot.new()
      text(0.5, 0.5, paste("Error:", e$message), col = "red")
  ...
  output$centers_heatmap <- shiny::renderPlot({
    ...
    tryCatch({
      # Use native plot_contributions method from KmeansVariables
      rv$model$plot_contributions(top_n = 10)
    }, error = function(e) {
      plot.new()
      text(0.5, 0.5, paste("Error:", e$message), col = "red")
    })
